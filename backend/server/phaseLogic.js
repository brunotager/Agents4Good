...
function extractSGA(msg) {
  ...
  if (earningsMatch) result.monthly_earnings = parseInt(earningsMatch[1].replace(/,/g, ''));
  const weeklyMatch = lower.match(/\$\s*([\d,]+)\s*(a |per )?(week|wk)/);
  if (weeklyMatch) result.monthly_earnings = Math.round(parseInt(weeklyMatch[1].replace(/,/g, '')) * 4.33);

  const hoursMatch = lower.match(/(\d+)\s*hours?\s*(a |per )?(week|wk)/);
  if (hoursMatch) result.hours_per_week = parseInt(hoursMatch[1]);
